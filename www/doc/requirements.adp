...
by <a href="mailto:jbank\@arsdigita.com">Joseph Bank</a> based
largely on the ACS Repository requirements written by <a href="mailto:tnight\@arsdigita.com">Todd Nightingale</a><i>This is a DRAFT</i><h3>I. Introduction</h3><p>OpenACS 4.x has a file storage module, so an obvious question
...
appropriate.</p><h3>II. Vision Statement</h3><p>There are thousands of independent developers all over the world
...
development collaboration.</p><h3>III. System/Application Overview</h3><p>The OpenACS download package provides an application for
managing file distribution.</p><p>The package consists of the following components:</p><ul>
...
parameter).</p><p>Don Downloader is scanning through the most recently uploaded
...
from the repository url).</p><p>Benny Beancounter loves to learn about who's downloading files
...
downloaded the file and their specified reason for downloading.</p><h3>V. Related Links</h3><ul>
<li>Design Document - <em>not present</em>.</li><li><a href="features.txt">Some Features</a></li>
</ul><h3>VI.A Requirements: Datamodel</h3><p><strong>10.0 Versioned File Storage</strong></p><p>OpenACS Download must provide versioned file storage.</p><p><strong>20.0 User Tracking</strong></p><p>OpenACS Download must store information about which users have
downloaded which files (including versions).</p><p><strong>20.0 Package Based Meta Information</strong></p><p>OpenACS Download must be able to store arbitrary meta
information on a per package basis. i.e. All files provided by this
instance of the package require the fields x, y and z.</p><h3>VI.B Requirements: Users Interface</h3>
...
<p>
<strong>100.0 Define a Package</strong> (must be logged in)</p><blockquote>
...
<strong>120.0 Upload Versions</strong> (must be logged in)</p><blockquote>
...
<strong>130.0 APM Auto-load</strong> (must be logged in)</p><blockquote>
...
</p><p>A logged in user must be able to comment on vendors, packages,
and versions.</p><h3>VI.C Requirements: Administrator's Interface</h3>
...
</blockquote><p><strong>210.0 Version Approval</strong></p><p>The administrator should be able to approve or reject any
submitted package version, and enter a comment as to why the
version was rejected or approved.</p><h3>VII. Revision History</h3><table cellpadding="2" cellspacing="2" width="90%" bgcolor="#EFEFEF">
<tr bgcolor="#E0E0E0">
...
</table><hr><address><a href="mailto:jbank\@arsdigita.com"></a></address>
